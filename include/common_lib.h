...
struct LidarFrame {
    ...
    cv::Mat img_dx;
    cv::Mat img_dy;
    PointCloudXYZI::Ptr points_corrected;
    std::vector<M4D> T_Li_Lk_vec;
...
template<typename T>
double getSubPixelValue(const cv::Mat& img, double x, double y) 
{
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    ...
    return T((1 - xx) * (1 - yy) * img.ptr<T>(y_lower)[x_lower] + xx * (1 - yy) * img.ptr<T>(y_lower)[x_upper] +
        (1 - xx) * yy * img.ptr<T>(y_upper)[x_lower] + xx * yy * img.ptr<T>(y_upper)[x_upper]);
}

#endif // COIN_LIO_COMMON_LIB_H_